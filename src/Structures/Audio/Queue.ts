...
export class Queue {
	...
	public constructor(public readonly store: QueueStore, public readonly guildID: string) {
		this.keys = {
			next: `bolt.${this.guildID}.n`, // stored in reverse order: right-most (last) element is the next in queue
			position: `bolt.${this.guildID}.p`,
			current: `bolt.${this.guildID}.c`, // left-most (first) element is the currently playing track,
			skips: `bolt.${this.guildID}.s`,
			systemPause: `bolt.${this.guildID}.sp`,
			replay: `bolt.${this.guildID}.r`,
			volume: `bolt.${this.guildID}.v`,
			text: `bolt.${this.guildID}.t`
		};
	}
	...
	public async pause({ system = false } = {}): Promise<void> {
